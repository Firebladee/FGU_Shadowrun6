...
function checkVisibility()
  ...
  if b == "1" then
    ...
    window.spells.setVisible(true);
  else
    window.spellsframe.setVisible(false);
    window.spells.setVisible(false);
  end
end
...
function parseComponents()
	...
	for i = 1, #aClauses do
		local nStarts, nEnds, sDesc = string.find(aClauses[i], "(%w[%w%s]*)");
		local sLabel = "";
		if nStarts then
			-- Calculate modifier based on mod value and sign value, if any
			local nAllowRoll = 0;
			local nMod = 0;
			local rSpellData = nil;
			if sDesc ~= nil and sDesc ~= "" then
        rSpellData = DataCommon.spelldata[sDesc];
        if rSpellData then
          rSpellData.sName = sDesc;
          sLabel = "Spellcasting";
          nAllowRoll = 1;      
          nMod = window.skills.getSkillMod(sLabel);
        else
--          Debug.console("parseComponents: ", sDesc, "not found in spelldata");
        end
 			end

			-- Insert the possible skill into the skill list
			table.insert(aComponents, {nStart = aClauseStats[i].startpos, nLabelEnd = aClauseStats[i].startpos + nEnds, nEnd = aClauseStats[i].endpos, sLabel = sLabel, nMod = nMod, nAllowRoll = nAllowRoll, rSpellDetail = rSpellData });
		end
	end
...
function onHoverUpdate(x, y)
	...
	for i = 1, #aComponents, 1 do
        ...
        rDragDetail = aComponents[i].rSpellDetail; 
...
function action(draginfo)
	if sDragLabel then
	  local nMagic = window.getDatabaseNode().getChild("base.attribute.magic.check").getValue();
	  rDragDetail.nMagic =  nMagic;
