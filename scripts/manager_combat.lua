...
function nextActor(bSkipBell, bNoRoundAdvance)
	...
	local nodeActive = getActiveCT();
	local np = NodeManager.get(nodeActive, "ctpassleft", 1);
	NodeManager.set(nodeActive, "ctpassleft", "number", np-1);
		...
		if nodeActive then
			for i = 1,#aEntries do
				...
				repeat
				  nodeNext = aEntries[i];
				  if nodeNext then
					-- check if got any passes left
					local np = NodeManager.get(nodeNext, "ctpassleft", 1);
					if np > 0 then
					  bFound = true;
					else
					  i = i+1;
			...
			until (bFound == true) or (i > #aEntries);
...
function nextPass(nPasses)
	...
	for i = nStartCounter, nPasses do
		...
		-- Update pass counter
		local nCurrent = DB.getValue(CT_PASS, 0) + nPasses;
		DB.setValue(CT_PASS, "number", nCurrent);
	
		-- Look if any actors have this many initiative passes. If not, Next Go to next Round.
		local bFound = false;
		for i = 1,#aEntries do
		local np = NodeManager.get(aEntries[i], "ctpassleft", 0);
		if (aEntries[i] == nodeActive) then
			if (np > 0) then 
			bFound = true;
			end
		...
		-- Announce new round
	local msg = {font = "narratorfont", icon = "turn_flag"};
	msg.text = "[" .. Interface.getString("combat_tag_pass") .. " " .. nCurrent .. "]";
	Comm.deliverChatMessage(msg);
	
	local bSkipBell = (nPasses > 1);
